...
function makeStringTransform(transformName, options, transformFn) {
  ...
  transform = function (file, config) {
    ...
    configData =
      transform.configData != null
        ...
        : loadTransformConfigSync(transformName, file, options);
    if (config != null) {
      configData =
  ...
  transform.setConfig = function (config, configOptions) {
    ...
    configDir =
      configOptions.configDir || (configFile ? path.dirname(configFile) : null);
    if (!config) {
...
function makeRequireTransform(transformName, options, transformFn) {
  ...
  transform = makeFalafelTransform(
    transformName,
    options,
