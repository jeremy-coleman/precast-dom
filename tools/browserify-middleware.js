...
var {browserify , watchify} = require("./browserify");
...
function ms(val) {
  ...
  if (typeof val === "string" && val.length > 0) {
    ...
    return function () {
      ...
      switch (msAbs) {
        case msAbs >= T.d:
          return Math.round(val / T.d) + "d";
        case msAbs >= T.h:
          return Math.round(val / T.h) + "h";
        case msAbs >= T.m:
          return Math.round(val / T.m) + "m";
        case msAbs >= T.s:
          return Math.round(val / T.s) + "s";
        default:
          return val + "ms";
      }
    };
...
function normalize(options) {
  ...
  if (options.cache === "dynamic") {
    ...
    options.cache =
      (options.cache.private ? "private" : "public") +
      ", max-age=" +
  ...
  if (Array.isArray(path)) {
    for (var i = 0; i < path.length; i++) {
      if (typeof path[i] === "object") {
        // obj spec support; i.e. {"jquery": {options...}}
        var spec = path[i];
...
function prepareResponse(body, headers, options) {
  ...
  var result = new Promise(function (resolve, reject) {
    ...
    return new PreparedResponse(body, options.gzip !== false ? gzippedBody : null, headers, options);
  });
...
function getResponse(bundle, options) {
  ...
  var response = getSource(bundle, options)
    .then(function (src) {
  ...
  return {
    send: function (req, res, next) {
      if (syncResponse) return syncResponse.send(req, res);
        ...
        return response.done(function (response) {
          response.send(req, res);
        }, next);
    },
    dispose: noop,
...
function minify(str, options) {
  ...
  //@ts-ignore
  const result = require("terser").minify(str, options);
  if (result.error) {
    throw result.error;
  }
  return result;
...
function browserify_middleware(filepath, options) {
  if (Array.isArray(filepath)) {
    return modules(filepath, options);
  }
  filepath = path.resolve(filepath);
  options = settings.normalize(options);
  options.noParse = options.noParse.map(function (filepath) {
    if (filepath[0] != ".") return filepath; //support `['jquery']` as well as `['./src/jquery.js']`
    return path.resolve(filepath);
  });
  if (fs.statSync(filepath).isDirectory()) {
    return directory(filepath, options);
  } else {
    return file(filepath, options);
  }
}
...
function modules(modules, options) {
  ...
  if (options.external) {
    options.external = options.external.filter(function (name) {
...
class BrowserifyMiddleware {
  static middleware = browserify_middleware
